...
import classes from './olumFisi.scss';
...
class olumFisi extends Component {
    state = {
        data: null,
        receiptNumber: "",
    ...
    postSearchButton = async () => {
        const postKayitNo = {
            kayitNo: this.state.receiptNumber //  31,
        };
        await axios.put('/novifarm/GetOlumKayit', postKayitNo)
            .then(async response => {
        ...
        if (this.state.notFound) {
            ...
            await axios.put('/novifarm/GetOlumKayit', postIrsaliyeNo)
            .then(async response => {
                //console.log(response);
                this.setState({
                    data: response.data,
    ...
    updateDataButton = async () => {
        const post = {
            kayitNo: this.state.receiptNumberBlocked,
            integration: this.state.integration,
            integrationState: this.state.integrationState
        };

        await axios.put('/novifarm/UpdateOlumKayit', post)
            .then(async res => {
                this.setState({
                    error: false,
    ...
    render() {
        if (this.state.loading && !this.state.error) {
            receiptNumberBlocked = this.state.data[0].kayitNo;
        ...
        return (
            <div className={classes.olumFisi}>
            {this.state.error && this.state.receiptNumber !== null 
                ...
                <div className={classes.olumFisi__section_about}>
                ...
                <div className={classes.olumFisi__item__0}>
                    <span>Kayıt No Giriniz</span>
                </div>
                <div className={classes.olumFisi__item__1}>
                    <ReceiptNoInputText
                ...
                <div className={classes.olumFisi__item__2}>
                    <SearchButton click={this.postSearchButton} />
                </div>
                <div className={classes.olumFisi__item__3}>
                    <span>IntegrationState:</span>
                </div>
                <div className={classes.olumFisi__item__4}>
                    <span>Integration:</span>
                </div>
                <div className={classes.olumFisi__item__5}>
                    <span>Kayıt No:</span>
                </div>
                <div className={classes.olumFisi__item__6}>
                    <ReceiptNoInputText
                        disabled="true"
                        receiptNumberBlocked={this.state.receiptNumberBlocked} />
                </div>
                <div className={classes.olumFisi__item__7}>
                    <ReceiptNoInputText
                        status={this.state.integrationState}
                        changed={this.integrationStateChancedHandler} />
                </div>
                <div className={classes.olumFisi__item__8}>
                    <ReceiptNoInputText
                        status={this.state.integration}
                        changed={this.integrationChancedHandler} />
                </div>
                <div className={classes.olumFisi__item__9}>
