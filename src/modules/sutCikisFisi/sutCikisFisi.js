...
import classes from './sutCikisFisi.scss';
...
class sutCikisFisi extends Component {
    state = {
        data: null,
        receiptNumber: "",
    ...
    postSearchButton = async () => {
        ...
        await axios.put('/novifarm/getsutcikisfisi', postfisNo)
            .then(async response => {
        ...
        if (this.state.notFound) {
            ...
            await axios.put('/novifarm/getsutcikisfisi', postIrsaliyeNo)
            .then(async response => {
                //console.log(response);
                this.setState({
                    data: response.data,
    ...
    updateDataButton = async () => {
        ...
        await axios.put('/novifarm/updatesutcikisfisi', post)
            .then(async res => {
                this.setState({
                    error: false,
    ...
    render() {
        ...
        return (
            <div className={classes.sutCikisFisi}>
            {this.state.error && this.state.receiptNumber !== null 
                ...
                <div className={classes.sutCikisFisi__section_about}>
                    <div className={classes.u_center_text}>
                        <h2 className={classes.heading_secondary}>
                            Novifarm Süt Çıkış Fişi
                        </h2>
                    </div>
                </div>
                <div className={classes.sutCikisFisi__item__0}>
                    <span>Fiş No Giriniz</span>
                </div>
                <div className={classes.sutCikisFisi__item__1}>
                    <ReceiptNoInputText
                ...
                <div className={classes.sutCikisFisi__item__2}>
                    <SearchButton click={this.postSearchButton} />
                </div>
                <div className={classes.sutCikisFisi__item__3}>
                    <span>IntegrationState:</span>
                </div>
                <div className={classes.sutCikisFisi__item__4}>
                    <span>Integration:</span>
                </div>
                <div className={classes.sutCikisFisi__item__5}>
                    <span>Fiş No:</span>
                </div>
                <div className={classes.sutCikisFisi__item__6}>
                    <ReceiptNoInputText
                        disabled="true"
                        receiptNumberBlocked={this.state.receiptNumberBlocked} />
                </div>
                <div className={classes.sutCikisFisi__item__7}>
                    <ReceiptNoInputText
                        status={this.state.integrationState}
                        changed={this.integrationStateChancedHandler} />
                </div>
                <div className={classes.sutCikisFisi__item__8}>
                    <ReceiptNoInputText
                        status={this.state.integration}
                        changed={this.integrationChancedHandler} />
                </div>
                <div className={classes.sutCikisFisi__item__9}>
